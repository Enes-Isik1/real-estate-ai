...
export default function NewDealPage() {
  ...
  const addFiles = (newFiles: File[]) => {
    ...
    const pdfFilesOnly = newFiles.filter(file => file.type === "application/pdf" || file.name.endsWith(".pdf"))
  ...
  const handleStartAnalysis = async () => {
    if (rawFiles.length === 0) {
      alert("Bitte lade mindestens eine PDF-Datei hoch, um die Analyse zu starten.")
      return
    }
  ...
  return (
    <div className="max-w-[1000px] mx-auto pb-24 p-6 md:p-8 space-y-8 animate-fade-in-up">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-200/60 pb-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Create New Deal</h1>
          <p className="text-gray-400 text-sm mt-1">
            Trage die Details ein, lade Dokumente hoch und lass die KI dein Profil sofort analysieren.
          </p>
        </div>
      </div>
      ...
      {!isSuccess ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white border border-gray-200/60 rounded-2xl shadow-sm p-6 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-900 text-lg">Inquiry Email einfügen</h3>
              </div>
            ...
            <div className="bg-white border border-gray-200/60 rounded-2xl shadow-sm p-6 space-y-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-gray-900 text-lg">Interne Notizen</h3>
              </div>
              <div>
                <textarea 
                  value={notes}
          ...
          <div className="space-y-6 lg:sticky lg:top-24">
            ...
            <div className="bg-white border border-gray-200/60 rounded-2xl shadow-sm p-6 space-y-4">
              <h3 className="font-bold text-gray-900 text-lg">Dokumente (PDF)</h3>
              ...
              >
                <input 
                  ...
                  className="hidden"
                />
                <UploadCloud className={`w-10 h-10 mx-auto transition-transform duration-200 ${
                  isDragging ? "text-indigo-600 scale-110" : "text-gray-400"
                }`} />
                <p className="mt-2 text-sm font-bold text-gray-700">Zieh die PDF-Datei hierhin</p>
                <p className="text-xs text-gray-400 mt-1">oder klicke, um deine Dateien zu durchsuchen</p>
              </div>
            ...
            <div className="space-y-4">
              {analysisError && (
                <p className="text-red-500 text-xs font-semibold bg-red-50 border border-red-100 rounded-xl p-3">{analysisError}</p>
              )}
              ...
              >
                <Sparkles className={`w-5 h-5 text-indigo-200 ${isAnalyzing ? "animate-spin" : "group-hover:scale-110 transition-transform"}`} />
                <span>{isAnalyzing ? "Analysiere Dokument..." : "Analyse jetzt starten"}</span>
              </button>
            </div>
          </div>
        ...
        <div className="bg-white border border-gray-200/60 rounded-3xl shadow-lg p-10 max-w-2xl mx-auto text-center space-y-6 py-16">
          ...
          <div className="space-y-2">
            <h2 className="text-2xl font-black text-gray-900 tracking-tight">Deal erstellt & analysiert!</h2>
            <p className="text-gray-400 text-sm">
              Die KI hat das PDF-Dokument erfolgreich ausgelesen und strukturiert.
            </p>
          </div>
          <div className="pt-4">
            <button 
