...
export function AnalysisProvider({ children }: { children: React.ReactNode }) {
  ...
  const positiveFindingsStrings = (analysis?.positiveFindings || []).map((f: any) => 
  ...
  const startAnalysis = async () => {
    ...
    // 1. Validierung: Dateiformat und Dateigröße prüfen
    ...
    for (const file of selectedFiles) {
      ...
      if (file.size > MAX_SIZE) {
        setError(`Die Datei "${file.name} " ist zu groß. Das Maximum liegt bei 10 MB.`);
        return;
      }
    }
    ...
    try {
      ...
      // Detailliertes Abfangen von Backend-Fehlern statt generellem "Error"
      if (!response.ok) {
        const errorData = await response.json().catch(() => null);
        throw new Error(
          errorData?.error || "Die Analyse konnte nicht abgeschlossen werden. Bitte versuchen Sie es erneut."
        );
      }
      ...
      // Wenn es ein Netzwerkfehler ist (z.B. Server offline)
...
export function useAnalysis() {
  ...
  if (!context) {
    throw new Error("useAnalysis muss innerhalb eines AnalysisProviders verwendet werden.");
  }
  return context;
}